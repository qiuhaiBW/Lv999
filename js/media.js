...
const MediaHub = (function () {
  ...
  async function fetchRemoteManifest(config) {
    ...
    const [musicItems, imageItems, thumbItems, videoItems] = await Promise.all([
      ...
      fetchGithubFolder(remote.repo, branch, `${imagesPath}/_thumb`),
    ...
    let images = imageItems
      ...
      .map((item) => {
        ...
        const thumbPath = `${imagesPath}/_thumb/${thumbFile}`;
  ...
  async function loadManifest() {
    ...
    manifest = {
      ...
      images: pickMediaList(localManifest.images, remoteManifest?.images),
